...
export function BudgetProgress({ categories, expenses }: { categories: Category[]; expenses: Expense[] }) {
  ...
  return (
    <Card style={{ gap: theme.spacing.md }}>
      ...
      {budgets.map((category) => {
        const spent = expenses.filter((expense) => expense.category_id === category.id && expense.date.startsWith(month)).reduce((total, expense) => total + Number(expense.amount), 0);
        ...
        return (
          <View key={category.id} style={{ gap: theme.spacing.xs }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              ...
              <Text variant="caption" style={{ color: overBudget ? theme.colors.danger : theme.colors.textMuted }}>{formatMoney(spent)} / {formatMoney(budget)}</Text>
            </View>
            ...
            {overBudget ? <Text variant="caption" style={{ color: theme.colors.danger }}>Over budget by {formatMoney(spent - budget)}.</Text> : null}
          </View>
        );
      })}
    </Card>
